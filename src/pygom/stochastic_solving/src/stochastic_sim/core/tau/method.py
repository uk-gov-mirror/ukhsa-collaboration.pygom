...
class Cao2006(Adaptive):
    ...
    def compute_tau(self, x, t, rates, thresholds):
        ...
        if not np.isfinite(tau):
            raise RuntimeError("Tau selection failed: Infinite step size calculated")
        if tau == 0:
            raise RuntimeError("Tau selection failed: Zero step size calculated")
        if tau < 0:
            raise RuntimeError("Tau selection failed: Negative step size calculated")
